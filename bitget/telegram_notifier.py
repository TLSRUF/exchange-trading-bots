...
# Bitget 주문 API의 side("buy"/"sell")는 그대로 두고, 텔레그램 알림에만 Long/Short으로 표시.
# webhook_bot.py의 헤지모드 청산 로직상 side="buy"는 항상 롱 관련(진입 또는 롱 청산), side="sell"은
# 항상 숏 관련(진입 또는 숏 청산)이 되도록 맞춰져 있어서 이 단순 매핑이 성립함 — 그 불변조건이
# 깨지면(예: 원웨이 모드로 바뀌는 등) 이 매핑도 같이 재검토 필요.
_SIDE_DISPLAY = {"buy": "Long", "sell": "Short"}


def notify_order_success(action_desc: str, symbol: str, side: str, size: str,
                          price: str = "", dry_run: bool = False) -> None:
    prefix = "🧪 [모의]" if dry_run else "✅ [실거래]"
    direction = _SIDE_DISPLAY.get(side, side)  # 매핑에 없는 값이면 원본 그대로 표시(안전장치)
    lines = [
